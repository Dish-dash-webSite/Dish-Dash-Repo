...
const Sidebar: React.FC = () => {
    ...
    const [isAvatarVisible, setIsAvatarVisible] = useState<boolean>(false); // State to control avatar visibility
    ...
    // Toggle the avatar visibility when the profile is clicked
            ...
            <nav className="mt-4">
                {menuItems.map((item, index) => (
                    <Link
                        key={index}
                        to={item.path}
                        className={`flex items-center p-4 hover:bg-[#334155] ${location.pathname === item.path ? 'bg-[#334155]' : ''} ${isOpen ? 'justify-start' : 'justify-center'}`}
                    >
